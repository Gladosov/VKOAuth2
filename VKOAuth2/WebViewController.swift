...
class WebViewController: UIViewController {
    ...
    var initURL: URL!
    var redirectURL: URL!
    var completion: (_ accessToken: Result<(token: String, expiresDate: Date, userID: Int), Error>) -> Void = { _ in }
...
extension WebViewController: WKNavigationDelegate {
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
        ...
        if redirectedUrl == redirectURL {
            ...
            guard let token = queryItems.filter({ $0.name == "access_token"}).first?.value else { return }
            guard let dateString = queryItems.filter({ $0.name == "expires_in"}).first?.value else { return }
            guard let userIDItem = queryItems.filter({ $0.name == "user_id"}).first?.value, let userID = Int(userIDItem)  else { return }
            let date = Date(timeIntervalSinceNow: Double(dateString)!)
            completion(.success((token, date, userID)))
